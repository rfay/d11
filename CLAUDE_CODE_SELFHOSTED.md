...
name comes from its directory name, `d11`, so it matches. The Coder app for
...
# Commits made in sessions need an identity; the workspace has none by default
# and `git commit` fails with "Author identity unknown".
...
until someone opens a terminal**. The freeform template has no user startup
hook for running it at workspace start.
...
`.ddev/docker-compose.coder-describe.yaml`, adds both to your global git
ignore, and adds a post-start hook that publishes Traefik routes for the Coder
URLs. `ddev start` then prints lines like `+ d11: http-8080 → d11-web-80`. If
...
- **You:** `https://d11--<workspace>--<owner>.coder.ddev.com`, and Mailpit at
  `https://mailpit-d11--<workspace>--<owner>.coder.ddev.com`. Coder
  authenticates you, so only the workspace owner (and people it's shared
  with) can open them.
...
- **Site redirects to `/core/install.php`:** the database is empty; install
  Drupal as above.
- **`Author identity unknown` on commit:** set `git config --global user.name`
...
The workspace used for testing:

| Setting | Value |
| --- | --- |
| Coder workspace / owner | `claude-d11-selfhosted` / `rfay` |
| Coder URL | `https://d11--claude-d11-selfhosted--rfay.coder.ddev.com` |
| Checkout | `~/workspace/rfay/d11` |
| Environment key | `~/.claude-d11-selfhosted-secret.txt` (symlinked as `~/.claude-runner/environment-secret`) |
