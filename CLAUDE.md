...
- **Anthropic-hosted sandbox:** `id -un` is `root` and the checkout is
  `/workspace/d11`. Follow "Claude Code cloud environment" below.
- **Self-hosted runner in a Coder workspace:** `id -un` is `coder` and the
